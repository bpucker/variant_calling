...
def load_variants_per_gene( navip_vcf_file ):
	"""! @brief load dNdS status per variant per gene from given VCF file """
	...
	with open( navip_vcf_file, "r" ) as f:
		...
		while line:
			if line[0] != '#':
				...
				dS_status = True	#mutation is a synonymous substitution (no change in encoded amino acid)
				...
				try:
					variants_per_gene[ gene_id ].append( dS_status )
				except KeyError:
					variants_per_gene.update( { gene_id: [ dS_status ] } )
			line = f.readline()
	return variants_per_gene


def calculate_dNdS_ratio_per_gene( variants_per_gene ):
	"""! @brief calculate dN/dS ratio per gene """
	
	dNdS_ratio_per_gene = {}
	for gene in list( variants_per_gene.keys() ):
		variants = variants_per_gene[ gene ]
		dN = float( len( variants ) - sum( variants ) )	#all variants minus synonymous ones
		dS = sum( variants )
		if dS > 0:
			dNdS_ratio_per_gene.update( { gene: dN / dS } )
		else:	#add one ot each of them to calculate a ratio
			dS += 1
			dN += 1
			dNdS_ratio_per_gene.update( { gene: dN / dS } )
	return dNdS_ratio_per_gene


def plot_dNdS_ratios_of_groups( dNdS_ratio_per_gene, genes, figure_file, supplied_genes, display_cutoff=10 ):
	...
	fig.update_layout(yaxis_title="dN/dS per gene")
...
def main( arguments ):
	...
	navip_vcf_file = arguments[ arguments.index( '--in' )+1 ]	#NAVIP output VCF file for dN/dS determination
	...
	# --- determine dN/dS ratios per gene --- #
	dNdS_ratio_per_gene = calculate_dNdS_ratio_per_gene( variants_per_gene )
	
	# --- plot ratios of group specified by input file against all other genes --- #
	figure_file = output_folder + "violin_plot.png"
	group1, group2 = plot_dNdS_ratios_of_groups( dNdS_ratio_per_gene, genes, figure_file, supplied_genes )
